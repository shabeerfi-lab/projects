...
*/
  ...
  SELECT [CUST_NO],[PRODUCT],[MPM_TLN_ACCT_NO],[DDA_ACCOUNT_NO],[DDA_SUB_PROD_CDE]
      ,CAST([TLN_TAKE_UP_DATE]AS DATE) AS [TLN_TAKE_UP_DATE],[TLN_LOAN_VALUE],[TLN_INIT_FEE],[TLN_TOTAL_AMT],[AMT_REPAID]
      ,[ARREARS],CAST([FORCE_PAY_DATE] AS DATE)AS[FORCE_PAY_DATE],[TLN_REMAINING_AMT],[ARREARS_DAYS],[TLN_TAKEUP_SOURCE],[STATUS]
	  ,RESIDUAL_ACC_NR,CONVERT(date,RESIDUAL_OPEN_DATE,105) AS RESIDUAL_OPEN_DATE
  FROM [App_Recoveries].[Staging].[ImportTLNResidual]
  WHERE [RESIDUAL_ACC_NR] NOT IN
  (SELECT 
      RESIDUAL_ACC_NR
    
  FROM [App_Recoveries].[dbo].RECOVERIES
  WHERE RESIDUAL_ACC_NR IS NOT NULL
  )
...
FETCH NEXT FROM @NewLiveAccounts INTO @CUST_NO,@PRODUCT,@MPM_TLN_ACCT_NO,@DDA_ACCOUNT_NO,@DDA_SUB_PROD_CDE,
									 ...
									 @Status,@RESIDUAL_ACC_NR,@RES_ACC_OPEN_DATE
...
SET @EDC_ID = (SELECT  [ID]  FROM [App_Recoveries].[dbo].[EDC] WHERE NAME =@Status)
...
SET @USER_ID = (SELECT ID FROM [Plexus].dbo.[USER] WHERE BUSINESS_AREA_ID =@BusinessAreaID AND USER_TYPE_ID=3)
...
--------------------END OF INSERT INTO CREDIT MESSAGE------------------------------------
 ...
 INSERT INTO [Plexus].[dbo].[WORK_ITEM]
 (
...
----END-----------------
 ...
 ----Insert WorkItem History----------
   INSERT INTO [Plexus].dbo.WORK_ITEM_HISTORY 
...
----Insert INTO  WORKITEM HISTORY NOTE------------
  INSERT INTO [Plexus].dbo.WORK_ITEM_HISTORY_NOTE
